...
3. Change directories into the new repo and checkout the local branch, the branch you'll use to get a local version of your Chrome Extension

   ``` cd Document-To-Calendar-Event-Extension; git checkout local ```
   
4. Change directories to the backend directory

   ``` cd backend ```
   
5. Install the @google/genai npm package

   ``` npm install @google/genai ```
   
6. Set up a local backend
